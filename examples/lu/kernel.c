...
void init_array(data_t A[N][N]) {
  ...
  for (i = 0; i < N; i++) {
    for (j = 0; j <= i; j++) A[i][j] = (data_t)(-j % N) / N + 1;
    for (j = i + 1; j < N; j++) {
      A[i][j] = 0;
    }
    A[i][i] = 1;
    ...
    for (int j = k; j < N; j++)
      for (int i = k; i < N; i++) {
        ...
        if (j == k) {
          ...
          V_tmp[i][j][k] =
              prev_V[i][j][k] - L_tmp[i][j][k] * U_tmp[i][j - 1][k];
        }
      }
}
...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < N; i++)
    for (int j = 0; j <= i; j++) {
      if (fabs((float)L_golden[i][j] - (float)L[i][j]) > 0.001) err++;
    }
  for (int i = 0; i < N; i++)
    for (int j = i; j < N; j++) {
      if (fabs((float)U_golden[i][j] - (float)U[i][j]) > 0.001) err++;
    }
