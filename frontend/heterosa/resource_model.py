...
def extract_design_info(design_dir, synth=0):
    ...
    if synth:
                ...
                if "local_buffers" in design_info["modules"][module]:
                    ...
                    for local_buffer in local_buffers:
                        if local_buffer["mem_type"] == "FF":
                            design_info["modules"][module][
                                "FF"
                            ] -= FF_array_predict_HLS(
                                local_buffer["port_width"], local_buffer["buffer_depth"]
                            )
        ...
        axi_bram, axi_ff, axi_lut = extract_axi_res_from_hls_rpt(
            f"{hls_rpts_dir}/{top_module_rpt_name}"
        )
...
def extract_resource_info_from_hls_rpt(rpt):
    ...
    """
    res = {"BRAM18K": 0, "DSP": 0, "URAM": 0, "FF": 0, "LUT": 0}
    root = rpt
        ...
        for module in modules:
            if module.find("IO") != -1:
                ...
                if module in ms:
                    ...
                    info_dict[module + "_data_pack_inter"].append(
                        ms[module]["data_pack_inter"]
                    )
                    info_dict[module + "_data_pack_intra"].append(
                        ms[module]["data_pack_intra"]
                    )
...
def df_feature_extract(df, module):
    ...
    """
    if module.find("IO") != -1:
        df[module + "_data_pack_inter/" + module + "_data_pack_intra"] = df.apply(
            lambda row: float(row[module + "_data_pack_inter"])
            / float(row[module + "_data_pack_intra"]),
            axis=1,
        )
...
def train(df, modules, fifos, design_infos, work_dir, logger):
    ...
    for module in modules:
        ...
        if X_test_module.shape[0] > 0:
            ...
            logger.info("======== FF ========")
            logger.info(
                f"Mean Absolute Error: {metrics.mean_absolute_error(y_test_module, y_pred_module)}"
            )
            logger.info(
                f"Mean Squared Error: {metrics.mean_squared_error(y_test_module, y_pred_module)}"
            )
            logger.info(
                f"Mean Absolute Percentage Error: {mean_absolute_percentage_error(y_test_module, y_pred_module)}"
            )
        ...
        if X_test_module.shape[0] > 0:
            ...
            LUT_mape.append(
                mean_absolute_percentage_error(y_test_module, y_pred_module)
            )
                    ...
                    for local_buffer in local_buffers:
                        if local_buffer["mem_type"] == "BRAM":
                            if "array_map" in local_buffer:
                                ...
                                BRAM_usage += (
                                    BRAM_array_predict_HLS(
                                        local_buffer["port_width"],
                                        local_buffer["buffer_depth"] * 2,
                                        local_buffer["partition_number"],
                                    )
                                    / 2
                                )
                            else:
                                BRAM_usage += BRAM_array_predict_HLS(
                                    local_buffer["port_width"],
                ...
                if "local_buffers" in design_info["modules"][module]:
                    ...
                    for local_buffer in local_buffers:
                        if local_buffer["mem_type"] == "URAM":
                            BRAM_usage += URAM_array_predict_HLS(
                                local_buffer["port_width"],
                                local_buffer["buffer_depth"],
                                local_buffer["partition_number"],
                            )
    ...
    logger.info(
        "FF Mean Absoulate Percentage Error (Arith. Mean): %.2f%%" % (mean(FF_mape))
    )
    logger.info(
        "LUT Mean Absoulate Percentage Error (Arith. Mean): %.2f%%" % (mean(LUT_mape))
    )
    logger.info(
        "DSP Mean Absoulate Percentage Error (Arith. Mean): %.2f%%" % (mean(DSP_mape))
    )
    logger.info(
        "BRAM18K Mean Absoulate Percentage Error (Arith. Mean): %.2f%%"
        % (mean(BRAM18K_mape))
    )
    logger.info(
        "URAM Mean Absoulate Percentage Error (Arith. Mean): %.2f%%" % (mean(URAM_mape))
    )
    ...
    for index, row in df_test.iterrows():
        ...
        res = predict_design_resource_usage(
            df_design, modules, fifos, design_info, work_dir
        )
        ...
        BRAM18K_mape = mean_absolute_percentage_error(
            float(design_info["BRAM18K"]), res["BRAM18K"]
        )
        URAM_mape = mean_absolute_percentage_error(
            float(design_info["URAM"]), res["URAM"]
        )
    ...
    logger.info(
        "FF Mean Absoulate Percentage Error (Arith. Mean): %.2f%%"
        % (mean(FF_design_mape))
    )
    logger.info(
        "LUT Mean Absoulate Percentage Error (Arith. Mean): %.2f%%"
        % (mean(LUT_design_mape))
    )
    logger.info(
        "DSP Mean Absoulate Percentage Error (Arith. Mean): %.2f%%"
        % (mean(DSP_design_mape))
    )
    logger.info(
        "BRAM18K Mean Absoulate Percentage Error (Arith. Mean): %.2f%%"
        % (mean(BRAM18K_design_mape))
    )
    logger.info(
        "URAM Mean Absoulate Percentage Error (Arith. Mean): %.2f%%"
        % (mean(URAM_design_mape))
    )


def predict_design_resource_usage(
    df,
    modules,
    fifos,
    design_info,
...
def unit_test_predict_design_resource(design_dir, hw_info, model_path):
    ...
    res = predict_design_resource_usage(df, modules, fifos, design_info, res_model_path)
    # compute the ratio
    print(f"FF: {res['FF']}/{hw_info['FF']} ({res['FF']/hw_info['FF']:.2f})")
    print(f"LUT: {res['LUT']}/{hw_info['LUT']} ({res['LUT']/hw_info['LUT']:.2f})")
    print(
        f"BRAM18K: {res['BRAM18K']}/{hw_info['BRAM18K']} ({res['BRAM18K']/hw_info['BRAM18K']:.2f})"
    )
