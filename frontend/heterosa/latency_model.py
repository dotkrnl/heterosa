...
def extract_latency_info(design_dir):
    ...
    ret = {
        "loop_infos": loop_info_all,
        "module_list": module_names,
        "module_grouped": module_grouped,
    ...
    if "loop" in loop_struct:
        ...
        if ub.isnumeric():
            ...
            raise NotImplementedError(
                f"Non-number loop upper bound ({ub}) is not supported."
            )
        ...
        if config["module_type"] == 1:
            if config["loop_prefix"] == "Loop":
                config["loop_prefix"] = config["loop_prefix"] + str(
                    config["loop_offset"]
                )
            else:
                config["loop_prefix"] = (
                    config["loop_prefix"] + "." + str(config["loop_offset"])
                )
    ...
    for module_name in module_grouped:
        ...
        if "inter_trans" in module or "intra_trans" in module:
            ...
            if module_loop_info["module_prop"]["double_buffer"] == 1:
                module_latency = outer_latency * max(
                    inter_trans_latency, intra_trans_latency
                )
