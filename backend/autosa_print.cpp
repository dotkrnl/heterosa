...
__isl_give isl_printer *autosa_array_ref_group_print_fifo_name(
  ...
  if (group->group_type == AUTOSA_PE_GROUP) return p;
 ...
 */
static int already_printed(struct autosa_types *types, struct pet_type *type) {
  int i;

  for (i = 0; i < types->n; ++i)
    if (!strcmp(types->name[i], type->name)) return 1;
...
__isl_give isl_printer *autosa_print_types(__isl_take isl_printer *p,
  ...
  for (i = 0; i < n; ++i) {
    ...
    if (already_printed(types, type)) continue;
...
__isl_give isl_printer *print_str_new_line(__isl_take isl_printer *p,
                                           const char *str) {
  p = isl_printer_start_line(p);
...
__isl_give isl_printer *autosa_print_array_type(
    __isl_take isl_printer *p, struct autosa_array_info *array) {
...
__isl_give isl_printer *autosa_kernel_print_domain(
...
__isl_give isl_printer *print_kernel_arguments(__isl_take isl_printer *p,
  ...
  for (i = 0; i < kernel->n_array; ++i) {
    ...
    if (required < 0) return isl_printer_free(p);
...
static isl_bool at_node(__isl_keep isl_ast_node *node, void *user) {
  ...
  if (isl_ast_node_get_type(node) != isl_ast_node_user) return isl_bool_true;

  id = isl_ast_node_get_annotation(node);
  if (!id) return isl_bool_false;

  name = isl_id_get_name(id);
  if (!name) return isl_bool_error;
  ...
  if ((is_kernel && !kernel) || (!is_kernel && !stmt)) return isl_bool_error;
...
static void print_iterators(FILE *out, const char *type,
  ...
  for (i = 0; i < n; ++i) {
    ...
    fprintf(out, "%s = %s", isl_id_get_name(id), dims[i]);
    isl_id_free(id);
  }
 ...
 * - the paramters
 * - the host loop iterators
 * - the input array accessed by the module (before
 * serialization/deserialization)
 * - the output array accessed by the module (after
 * serialization/deserialization)
 */
...
__isl_give isl_printer *autosa_fifo_print_declaration_arguments(
    ...
    const char *suffix) {
  p = print_fifo_type(p, group, n_lane);
  p = isl_printer_print_str(p, " &");
...
__isl_give isl_printer *print_module_arguments(__isl_take isl_printer *p,
  ...
  for (int i = 0; i < nparam; ++i) {
    ...
    if (types) p = isl_printer_print_str(p, "int ");
    if (!types) p = isl_printer_print_str(p, "/* param */ ");
    p = isl_printer_print_str(p, name);
  ...
  if (module->type != PE_MODULE && module->to_mem) {
    if (!module->is_serialized || (module->is_serialized && serialize)) {
      ...
      if (types) {
        p = autosa_array_info_print_declaration_argument(
            p, module->io_groups[0]->array, n_lane, NULL, -1);
      } else {
        p = isl_printer_print_str(p, "/* array */ ");
        p = autosa_module_array_info_print_call_argument(
    ...
    for (int i = 0; i < prog->n_array; i++) {
      ...
      if (autosa_array_is_read_only_scalar(&prog->array[i])) {
        ...
        if (types)
          p = autosa_array_info_print_declaration_argument(p, &prog->array[i],
                                                           1, NULL, -1);
        else {
    ...
    for (int i = 0; i < module->n_io_group; i++) {
      if (!module->to_mem && inter != 0) {
        if (!(!module->in && boundary)) {
          ...
          if (types)
            p = autosa_fifo_print_declaration_arguments(
                p, module->io_groups[i], module->data_pack_inter, "in");
          else {
            p = isl_printer_print_str(p, "/* fifo */ ");
            p = autosa_fifo_print_call_argument(p, module->io_groups[i], "in");
          }
        ...
        if (!(module->in && boundary)) {
          ...
          if (types)
            p = autosa_fifo_print_declaration_arguments(
                p, module->io_groups[i], module->data_pack_inter, "out");
          else {
            p = isl_printer_print_str(p, "/* fifo */ ");
            p = autosa_fifo_print_call_argument(p, module->io_groups[i], "out");
          }
      ...
      if (inter != 1) {
        ...
        if (types) {
          p = autosa_fifo_print_declaration_arguments(
              ...
              module->in ? "local_out" : "local_in");
        } else {
          p = isl_printer_print_str(p, "/* fifo */ ");
          p = autosa_fifo_print_call_argument(
              p, module->io_groups[i], module->in ? "local_out" : "local_in");
        }
        first = 0;
      }
    }
  ...
  if (module->double_buffer && inter != -1) {
    ...
    if (types) {
      p = isl_printer_print_str(
          p, inter == 0 ? "bool intra_trans_en" : "bool inter_trans_en");
    } else {
      p = isl_printer_print_str(p, "/* enable */ ");
      p = isl_printer_print_str(
          p, inter == 0 ? "intra_trans_en" : "inter_trans_en");
    }
...
/* Print the arguments to a pe dummy module declaration or call. If "types" is
 * set, then print a declaration (including the types of the arguments).
 *
 * The arguments are printed in the following order
 * - the module identifiers
 * - the parameters
...
__isl_give isl_printer *print_pe_dummy_module_arguments(
  ...
  for (int i = 0; i < prog->n_array; i++) {
    ...
    if (autosa_array_is_read_only_scalar(&prog->array[i])) {
      ...
      if (types)
        p = autosa_array_info_print_declaration_argument(p, &prog->array[i], 1,
                                                         NULL, -1);
      else
        p = autosa_module_array_info_print_call_argument(p, &prog->array[i]);
      first = 0;
    }
  }
...
static __isl_give isl_printer *print_top_gen_header(
    __isl_take isl_printer *p, struct autosa_prog *prog,
...
static __isl_give isl_printer *print_pretrans_inst_ids_suffix(
    __isl_take isl_printer *p, int n_id, __isl_keep isl_ast_expr *expr,
    __isl_keep isl_vec *offset) {
...
static __isl_give isl_printer *print_fifo_decl_single(
    __isl_take isl_printer *p, struct autosa_kernel_stmt *stmt,
    struct autosa_prog *prog, struct hls_info *hls, int pe_inout,
    const char *suffix) {
  ...
  if (module->type == IO_MODULE || module->type == DRAIN_MODULE) {
    ...
    if (boundary)
      p = print_pretrans_inst_ids_suffix(p, n, group->io_L1_pe_expr,
                                         group->dir);
    else
  ...
  p = isl_printer_start_line(p);
  p = isl_printer_print_str(
      p, "p = isl_printer_print_str(p, \"#pragma HLS STREAM variable=");
  p = autosa_array_ref_group_print_fifo_name(group, p);
  p = isl_printer_print_str(p, "_");
  p = isl_printer_print_str(p, module->name);
  if (pe_inout) {
  ...
  if (n_lane * group->array->size > 32) {
    ...
    p = isl_printer_start_line(p);
    p = isl_printer_print_str(
        p, "p = isl_printer_print_str(p, \"#pragma HLS RESOURCE variable=");
    p = autosa_array_ref_group_print_fifo_name(group, p);
    p = isl_printer_print_str(p, "_");
    p = isl_printer_print_str(p, module->name);
    if (pe_inout) {
    ...
    p = print_str_new_line(p,
                           "p = isl_printer_print_str(p, \" core=FIFO_SRL\");");
    p = print_str_new_line(p, "p = isl_printer_end_line(p);");
  }
...
static __isl_give isl_printer *print_fifo_decl(__isl_take isl_printer *p,
  ...
  if (group->io_type == AUTOSA_INT_IO && module->type == PE_MODULE &&
      group->pe_io_dir == IO_INOUT) {
    pe_inout = 1;
  } else {
    pe_inout = 0;
  }
...
static __isl_give isl_printer *print_fifo_annotation(
 ...
 */
static __isl_give isl_printer *print_fifo_prefix(
    __isl_take isl_printer *p, struct autosa_hw_module *module,
    struct autosa_array_ref_group *group) {
  p = isl_printer_start_line(p);
  p = isl_printer_print_str(p, "p = isl_printer_print_str(p, \"");
...
__isl_give isl_printer *print_module_call_upper(__isl_take isl_printer *p,
  ...
  if (isl_id_list_n_id(module->inst_ids) > 0 &&
      prog->scop->options->autosa->use_cplusplus_template) {
    p = print_str_new_line(p, "p = isl_printer_print_str(p, \"<\");");
    if (!dummy) {
  ...
  if (!prog->scop->options->autosa->use_cplusplus_template) {
    if (!dummy) {
      for (int i = 0; i < isl_id_list_n_id(module->inst_ids); i++) {
        ...
        p = isl_printer_start_line(p);
        p = isl_printer_print_str(
            p, "p = isl_printer_print_str(p, \"/* module id */ \");");
        p = isl_printer_end_line(p);
        p = isl_printer_start_line(p);
        p = isl_printer_print_str(p, "p = isl_printer_print_int(p, c");
        p = isl_printer_print_int(p, i);
  ...
  for (int i = 0; i < n; i++) {
    ...
    const char *name =
        isl_space_get_dim_name(module->kernel->space, isl_dim_set, i);
    p = isl_printer_start_line(p);
    p = isl_printer_print_str(
        p, "p = isl_printer_print_str(p, \"/* host iter */ ");
    p = isl_printer_print_str(p, name);
    p = isl_printer_print_str(p, "\");");
    p = isl_printer_end_line(p);
  }
  ...
  if (module->type != PE_MODULE && module->to_mem &&
    ...
    for (int i = 0; i < prog->n_array; i++) {
      ...
      if (autosa_array_is_read_only_scalar(&prog->array[i])) {
        ...
        p = isl_printer_start_line(p);
        p = isl_printer_print_str(
            p, "p = isl_printer_print_str(p, \"/* scalar */ ");
        p = isl_printer_print_str(p, module->io_groups[0]->array->name);
        p = isl_printer_print_str(p, "\");");
        p = isl_printer_end_line(p);
      }
  ...
  if (module->type == PE_MODULE) {
      ...
      for (int i = 0; i < module->n_io_group; i++) {
        ...
        if (group->pe_io_dir == IO_INOUT) {
          ...
          if (group->io_type == AUTOSA_INT_IO) {
            p = isl_printer_start_line(p);
            p = isl_printer_print_str(
                p, "p = isl_printer_print_str(p, \"_out\");");
            p = isl_printer_end_line(p);
          }
          if (group->io_type == AUTOSA_INT_IO) {
            p = print_inst_ids_suffix(p, n, NULL);
    ...
    if (!module->to_mem) {
      ...
      if (module->is_serialized && !serialize) {
        ...
        p = isl_printer_start_line(p);
        p = isl_printer_print_str(
            p, "p = isl_printer_print_str(p, \"_serialize\");");
        p = isl_printer_end_line(p);
      }
    }
  }
 ...
 */
static __isl_give isl_printer *print_module_call_lower(
    __isl_take isl_printer *p, struct autosa_kernel_stmt *stmt,
    struct autosa_prog *prog) {
  struct autosa_hw_module *module = stmt->u.m.module;
  int lower = stmt->u.m.lower;
  ...
  if (lower) {
    ...
    if (serialize) {
      ...
      if (group->io_type == AUTOSA_INT_IO && lower_is_PE &&
          group->pe_io_dir == IO_INOUT) {
        /* Add in/out suffix. */
        p = isl_printer_start_line(p);
        p = isl_printer_print_str(p, "p = isl_printer_print_str(p, \"");
      ...
      if (lower_is_PE) {
        p = print_pretrans_inst_ids_suffix(
            p, module->kernel->n_sa_dim,
...
__isl_give isl_printer *autosa_kernel_print_module_call(
  ...
  if (complete) {
    ...
    p = isl_printer_start_line(p);
    p = isl_printer_print_str(
        p, "p = isl_printer_print_str(p, \"/* Module Call */\");");
    p = isl_printer_end_line(p);
...
__isl_give isl_printer *autosa_kernel_print_io(__isl_take isl_printer *p,
  ...
  fifo_name =
      concat(ctx, stmt->u.i.in_fifo_name, stmt->u.i.in == 1 ? "in" : "out");
  ...
  if (data_pack > 1) {
    ...
    local_index_packed =
        isl_ast_expr_set_op_arg(local_index_packed, n_arg - 1, arg);
  }
    ...
    if (stmt->u.i.in) {
      ...
      if (nxt_data_pack == 1) {
        ...
        p = isl_printer_print_int(p,
                                  group->array->size * 8 * nxt_data_pack - 1);
        p = isl_printer_print_str(p, ", 0);");
        p = isl_printer_end_line(p);
      }
...
__isl_give isl_printer *autosa_print_reduce_data_pack(
    __isl_take isl_printer *p, struct autosa_kernel_stmt *stmt,
    int data_pack_in, int data_pack_out, struct autosa_array_ref_group *group) {
  ...
  for (int i = 0; i < data_pack_in; i++) {
    ...
    p = isl_printer_print_int(p, i);
    p = isl_printer_print_str(p,
                              ".ui = (unsigned int)buf_data_split[split_i](");
    p = isl_printer_print_int(p, group->array->size * 8 * (i + 1) - 1);
    p = isl_printer_print_str(p, ", ");
    p = isl_printer_print_int(p, group->array->size * 8 * i);
    p = isl_printer_print_str(p, ");");
  ...
  for (int i = data_pack_in - 1; i >= 0; i--) {
    if (i != data_pack_in - 1) p = isl_printer_print_str(p, ", ");
    p = isl_printer_print_str(p, "(ap_uint<");
...
__isl_give isl_printer *autosa_print_reduce_default(
    __isl_take isl_printer *p, struct autosa_kernel_stmt *stmt, int data_pack,
    isl_ast_expr *index, struct autosa_array_ref_group *group) {
  p = print_str_new_line(p, "/* Local Reduction */");
    ...
    if (data_pack == 1) {
      ...
      if (stmt->u.i.module->double_buffer &&
        ...
        throw std::runtime_error(
            "[AutoSA] Error: Local reduce for double buffer style 0 is not "
            "supported!");
      else {
  ...
  if (data_pack == 1) {
    ...
    for (int i = data_pack - 1; i >= 0; i--) {
      if (!is_first) p = isl_printer_print_str(p, ", ");
      p = isl_printer_print_str(p, "(ap_uint<");
      p = isl_printer_print_int(p, group->array->size * 8);
      p = isl_printer_print_str(p, ">)uout_");
      p = isl_printer_print_int(p, i);
...
static __isl_give isl_printer *autosa_kernel_print_io_transfer_default(
   ...
   */
  if (boundary) stmt->u.i.filter_sched_depth = -1;
  ...
  if (stmt->u.i.in) {
    ...
    if (stmt->u.i.buf) {
      /* local[][] = fifo_data; */
      if (stmt->u.i.reduce) {
        p = autosa_print_reduce_default(p, stmt, n_lane, local_index_packed,
                                        group);
      } else {
        p = isl_printer_start_line(p);
        if (stmt->u.i.module->double_buffer &&
          ...
          for (int n = 1; n < isl_ast_expr_op_get_n_arg(local_index_packed);
               n++) {
...
static __isl_give isl_printer *autosa_kernel_print_io_transfer_data_pack(
  ...
  if (stmt->u.i.in) {
    ...
    if (nxt_n_lane == 1) {
      ...
      p = isl_printer_start_line(p);
      p = isl_printer_print_str(
          p, "u.ui = (unsigned int)buf_data_split[split_i];");
      p = isl_printer_end_line(p);
    }
...
__isl_give isl_printer *autosa_kernel_print_io_dram(
  ...
  if (stmt->u.i.in) {
    ...
    if (module->is_serialized) {
      ...
      p = print_fifo_rw(p, serialize_fifo_name, 0);
      p = isl_printer_print_str(p, "fifo_data);");
...
static __isl_give isl_printer *print_inter_trans_module_call(
  ...
  p = isl_printer_indent(p, 2);
  p = isl_printer_start_line(p);
  p = print_module_arguments(p, prog, kernel, module, 0, 1, arb, boundary, 0);
  p = isl_printer_end_line(p);
  p = isl_printer_indent(p, -2);
  p = isl_printer_start_line(p);
  p = isl_printer_print_str(p, ");");
...
static __isl_give isl_printer *print_intra_trans_module_call(
  ...
  p = isl_printer_indent(p, 2);
  p = isl_printer_start_line(p);
  p = print_module_arguments(p, prog, kernel, module, 0, 0, arb, 0, 0);
  p = isl_printer_end_line(p);
  p = isl_printer_indent(p, -2);
  p = isl_printer_start_line(p);
  p = isl_printer_print_str(p, ");");
