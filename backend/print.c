...
static __isl_give isl_printer *print_ppcg_macro(__isl_take isl_printer *p,
  ...
  if (printed < 0) return isl_printer_free(p);
  if (printed) return p;
...
__isl_give isl_printer *ppcg_set_macros(__isl_take isl_printer *p,
  ...
  if (!macros) return isl_printer_free(p);
  macros->min = min;
  macros->max = max;
 ...
 * in "ref2expr".
 */
__isl_give isl_printer *ppcg_print_body_macros(
...
__isl_give isl_multi_pw_aff *ppcg_attach_multi_pw_aff(
  ...
  if (params < 0 || params) {
    ...
    if (params < 0) return isl_multi_pw_aff_free(mpa);
    return mpa;
  }
...
__isl_give isl_printer *ppcg_print_declaration_with_size(
  ...
  if (!base_type || !size) return isl_printer_free(p);
...
static __isl_give isl_printer *print_declarations(__isl_take isl_printer *p,
  ...
  for (i = 0; i < scop->pet->n_array; ++i) {
    ...
    if (!array->declared) continue;
    if (array->exposed != exposed) continue;
