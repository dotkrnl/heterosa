 ...
 * - uniform dependency
 */
isl_bool sa_legality_check(__isl_keep isl_schedule *schedule,
  ...
  single_band =
      (type == isl_schedule_node_band) ? isl_bool_true : isl_bool_false;
  ...
  if (all_uniform_dep < 1) {
    throw std::runtime_error(
        "[AutoSA] Error: Non-uniform dependence detected.");
  }
...
struct autosa_kernel **sa_space_time_transform_at_dim_async(
    __isl_keep isl_schedule *schedule, struct ppcg_scop *scop, isl_size dim,
    isl_size *num_sa) {
  ...
  if (dim == 1) {
    for (int i = 0; i < band_w; i++) {
      if (is_space_loop[i]) {
        ...
        sas = (struct autosa_kernel **)realloc(
            sas, (*num_sa + 1) * sizeof(struct autosa_kernel *));
        sas[*num_sa] = sa;
        *num_sa = *num_sa + 1;
      }
    }
        ...
        for (int j = i + 1; j < band_w; j++) {
          if (is_space_loop[j]) {
            ...
            struct autosa_kernel *sa =
                autosa_kernel_from_schedule(new_schedule);
            sa->scop = scop;
            sa->type = AUTOSA_SA_TYPE_ASYNC;
...
/* Apply space-time transformation to generate different systolic array
 * candidates. */
struct autosa_kernel **sa_space_time_transform(
    __isl_take isl_schedule *schedule, struct ppcg_scop *scop,
    isl_size *num_sa) {
  ...
  if (scop->options->autosa->max_sa_dim >= 1 && band_w >= 1) {
    ...
    isl_size n_sa_dim = 0;
    struct autosa_kernel **sa_dim_list =
        sa_space_time_transform_at_dim(schedule, scop, 1, &n_sa_dim);
    if (scop->options->autosa->verbose) {
      printf("[AutoSA] %d candidates generated.\n", n_sa_dim);
    }
    sa_list = (struct autosa_kernel **)realloc(
        sa_list, (n_sa + n_sa_dim) * sizeof(struct autosa_kernel *));
  ...
  if (scop->options->autosa->max_sa_dim >= 2 && band_w >= 2) {
    ...
    isl_size n_sa_dim = 0;
    struct autosa_kernel **sa_dim_list =
        sa_space_time_transform_at_dim(schedule, scop, 2, &n_sa_dim);
    if (scop->options->autosa->verbose) {
      printf("[AutoSA] %d candidates generated.\n", n_sa_dim);
    }
    sa_list = (struct autosa_kernel **)realloc(
  ...
  if (scop->options->autosa->max_sa_dim >= 3 && band_w >= 3) {
    ...
    isl_size n_sa_dim = 0;
    struct autosa_kernel **sa_dim_list =
        sa_space_time_transform_at_dim(schedule, scop, 3, &n_sa_dim);
    if (scop->options->autosa->verbose) {
      printf("[AutoSA] %d candidates generated.\n", n_sa_dim);
    }
    sa_list = (struct autosa_kernel **)realloc(
...
isl_stat sa_loop_init(struct autosa_kernel *sa) {
  ...
  root = isl_schedule_node_map_descendant_bottom_up(
      root, &init_band_node_sa_properties, sa);
 ...
 * else return true.
 */
static isl_bool not_carried_at_space(__isl_keep isl_schedule_node *node,
                                     void *user) {
  ...
  for (int i = 0; i < n_dim; i++) {
    if (isl_schedule_node_band_member_get_space_time(node, i) ==
        autosa_loop_space) {
      if (n_space_dim == 0) space_dim_start = i;
      n_space_dim++;
    }
  }
  ...
  if (n_space_dim > 0) {
    ...
    isl_vec *dirvec =
        isl_vec_zero(isl_schedule_node_get_ctx(node), n_space_dim);
    int carried = 0;
    for (int i = 0; i < n_space_dim; i++) {
      ...
      if (isl_val_get_num_si(val) > 0) carried = 1;
      isl_val_free(val);
    }
    data->dirvec = dirvec;
    isl_vec_free(disvec);
 ...
 */
static isl_bool sa_candidates_smart_pick_update(__isl_keep isl_map *map,
                                                void *user) {
 ...
 * Namely, for each dependnece, if it is a RAR carried by space or a RAW carried
 * by time loops, it will contriute one credit to the total score. Besides,
 * between 1D and 2D systolic arrays, we prefer 2D systolic arrays for now.
 */
struct autosa_kernel *sa_candidates_smart_pick(struct autosa_kernel **sa_list,
  ...
  for (int i = 0; i < num_sa; i++) autosa_kernel_free(sa_list[i]);
  free(sa_list);
 ...
 */
isl_stat data_transfer_update(__isl_keep isl_basic_map *dep,
                              struct data_transfer_opt_data *data) {
  ...
  if (is_carried_at_space) {
    access->io_info = (struct autosa_io_info **)realloc(
        access->io_info,
        sizeof(struct autosa_io_info *) * (++access->n_io_info));
    ...
    access->io_info[access->n_io_info - 1]->dep->isl_dep =
    ...
    access->io_info[access->n_io_info - 1]->old_dir =
        isl_vec_dup(internal_data.dirvec);
  } else {
    access->io_info = (struct autosa_io_info **)realloc(
        access->io_info,
        sizeof(struct autosa_io_info *) * (++access->n_io_info));
...
static isl_bool data_transfer_update_wrap(__isl_keep isl_map *map, void *user) {
  ...
  for (int i = 0; i < isl_map_n_basic_map(map); i++) {
    ...
    struct data_transfer_opt_data *opt_data =
        (struct data_transfer_opt_data *)user;
...
static isl_stat sa_io_update(struct autosa_kernel *sa) {
  ...
  for (int i = 0; i < sa->n_array; i++) {
    ...
    for (int j = 0; j < local_array->array->n_ref; j++) {
      ...
      struct data_transfer_opt_data opt_data = {access, sa, AUTOSA_DEP_UNKNOWN,
                                                isl_bool_false};
...
void extract_sa_dims_from_node(__isl_keep isl_schedule_node *node, int *sa_dims,
                               int n_sa_dim) {
  int *ubs;
 ...
 */
isl_stat sa_array_partitioning_optimize(struct autosa_kernel *sa, bool en,
                                        char *mode, bool L2_en, char *L2_mode) {
  int tile_len;
  ...
  if (sa->type == AUTOSA_SA_TYPE_SYNC) {
    ...
    isl_die(sa->ctx, isl_error_invalid, "systolic array type not supported",
  ...
  for (int i = 0; i < sa->n_sa_dim; i++) {
    if ((sa->type == AUTOSA_SA_TYPE_SYNC &&
         tile_size[tile_len - sa->n_sa_dim + i] == 1) ||
  ...
  if (!sa->options->autosa->credit_control) {
    for (int i = 0; i < isl_schedule_node_band_n_member(node); i++) {
      if (!isl_schedule_node_band_member_get_coincident(node, i)) {
        printf(
            "[AutoSA] Warning: Flow deps carried in the array partitioning "
            "band.\n");
        printf(
            "[AutoSA] Warning: Using simple task pipelining could lead to "
            "potential data hazards.\n");
        printf(
            "[AutoSA] Warning: The program will proceed as usual. You could "
            "consider enabling credit control.\n");
        break;
      }
    }
  } else {
  ...
  /* If two-level buffering is enabled, we will need to apply a second-level
   * tiling on the tile band from the previous array partitioning. Namely, after
   * array partitioning, we get two bands:
   * T
   * |
   * P
  ...
  if (sa->options->autosa->two_level_buffer) {
    if (L2_en) {
      /* Tile the band again */
      printf(
          "[AutoSA] Two-level buffering is set. Apply second-level array "
          "partitioning.\n");
      tile_len = isl_schedule_node_band_n_member(node);
      if (!strcmp(mode, "manual")) {
        tile_size = read_array_part_L2_tile_sizes(sa, tile_len);
        if (!tile_size) {
          /* Dump out the number of and upper bounds of array_part loops and
           * exit the program. */
          int *ubs = extract_band_upper_bounds(node);
...
static __isl_give isl_schedule_node *add_hls_pipeline(
  ...
  node = isl_schedule_node_child(node, 0);
  isl_bool no_inner_band =
      isl_schedule_node_every_descendant(node, &no_permutable_node, NULL);
  node = isl_schedule_node_parent(node);
  if (!no_inner_band) return node;

  int n = isl_schedule_node_band_n_member(node);

  if (sa->type == AUTOSA_SA_TYPE_ASYNC) {
    if (isl_schedule_node_band_member_get_space_time(node, n - 1) ==
        autosa_loop_time) {
      isl_id *id;
 ...
 */
static isl_schedule_node *detect_latency_hiding_loop(
    __isl_take isl_schedule_node *node, void *user) {
  struct autosa_kernel *sa = (struct autosa_kernel *)user;
...
/* Given each node band, tile the candidate loop and permute it innermost in the
 * time loop band. If the tile size is no greater than 1, the candidate loop is
 * skipped. For each point loop, a "latency" mark is added.
 */
  ...
  for (int i = n - 1; i >= 0; i--) {
    if (isl_schedule_node_band_member_get_pe_opt(node, i) ==
      ...
      if (loop_tile_size > 1) {
        ...
        node = isl_schedule_node_child(node, 0);
        node = isl_schedule_node_band_member_set_space_time(node, 0,
                                                            autosa_loop_time);
 ...
 */
static isl_bool count_latency_hiding_loop(__isl_keep isl_schedule_node *node,
                                          void *user) {
...
static __isl_give isl_schedule_node *autosa_latency_tile_loop(
  ...
  if (!strcmp(mode, "manual")) {
    ...
    if (!tile_size) {
      /* Dump out the number and upper bounds of latency loops and exit the
       * program. */
      int *ubs = data.ubs;
      FILE *fp;
      char *content;
  ...
  for (i = 0; i < tile_len; i++) {
    if (tile_size[i] != -1) sa->lat_hide_len *= tile_size[i];
  }
  for (i = 0; i < tile_len; i++) {
    if (tile_size[i] > 1) break;
  }
  if (i == tile_len) {
    node =
        isl_schedule_node_map_descendant_bottom_up(node, &add_hls_pipeline, sa);
  } else {
 ...
 * Go through all the loops, if there is any parallel loop (considering only
 * RAW), such a loop will be identified as latency hiding loop candidate. Such
 * loops will be tiled. The point loops will be permuted as the innermost time
 * loops.
 *
 * en: enable signal for the current stage.
 * mode: manual/auto
 */
isl_stat sa_latency_hiding_optimize(struct autosa_kernel *sa, bool en,
                                    char *mode) {
...
static isl_bool is_stride_coalesced_stmt(__isl_keep isl_set *set, void *user) {
  ...
  prefix = isl_map_from_union_map(isl_union_map_intersect_domain(
      isl_union_map_copy(data->prefix),
...
static isl_bool is_stride_coalesced_at_node(__isl_keep isl_schedule_node *node,
  ...
  one_or_zero =
      isl_union_set_every_set(domain, &is_stride_coalesced_stmt, data);
...
static float is_stride_coalesced(__isl_keep isl_schedule_node *node,
  ...
  coalesced = isl_schedule_node_every_descendant(
      node, &is_stride_coalesced_at_node, &data);
      ...
      for (int j = 0; j < local_array->array->n_ref; j++) {
        ...
        if (acc->layout_trans != -1) {
          if (acc->layout_trans == 1) {
            ...
            printf("\n");
            printf(
                "[AutoSA] Layout transform: Permute dim (%d) to the "
                "innermost\n",
                acc->simd_dim);
            *layout_transform = 1;
          }
      ...
      if (!simd_touch_space && isl_schedule_node_band_member_get_space_time(
        ...
        if (!isl_schedule_node_band_member_get_coincident(node, i) &&
          ...
          if (data->buffer == NULL) {
            ...
            isl_printer_free(p);
            printf(
                "[AutoSA] Please input if the current loop is a reduction loop "
                "[y/n]: ");
          }
          if (data->buffer == NULL) {
        ...
        if (is_parallel || is_reduction) {
          ...
          if (score_i < 0) {
            ...
            printf(
                "[AutoSA] -----------------------------------------------\n");
            printf(
                "[AutoSA] The loop is legal to be vectorized with score: %f\n",
                score);
            if (layout_transform)
              printf(
            ...
            data->scores =
                (float *)realloc(data->scores, sizeof(float) * data->n_loops);
...
/* Update the stride information for the array accesses under the SIMD loop.
 */
static isl_bool update_simd_acc(__isl_keep isl_schedule_node *node,
                                void *user) {
  isl_union_set *domain;
  isl_union_map *prefix;
  struct stride_coalesced_data *data = (struct stride_coalesced_data *)user;
 ...
 * Loops with tiling factors of one or require layout transformation are
 * skipped. At last, it will also update the stride information for the array
 * accesses under the SIMD loop.
 */
...
isl_stat sa_simd_vectorization_optimize(struct autosa_kernel *sa, char *mode) {
  ...
  if (data.n_legal_loops == 0) {
    ...
    if (!strcmp(mode, "manual")) {
      ...
      throw std::runtime_error(
          "[AutoSA] Error: Auto SIMD vectorization is not supported.\n");
    }
    ...
    node = isl_schedule_node_map_descendant_bottom_up(
        node, &autosa_simd_tile_loop, &data);
  }
...
isl_stat sa_pe_optimize(struct autosa_kernel *sa, bool pass_en[],
  ...
  sa->sizes =
      extract_sizes_from_str(sa->ctx, sa->scop->options->autosa->sa_sizes);
  /* Set the core */
...
static __isl_give isl_multi_pw_aff *extract_grid_size(
  ...
  for (i = 0; i < kernel->n_grid; ++i) {
    ...
    ls = isl_constraint_alloc_equality(
        isl_local_space_from_space(isl_set_get_space(grid)));
 ...
 * Lastly, I/O optimization optimizes the I/O system, performing tasks
 * including:
...
static void localize_bounds(struct autosa_kernel *kernel,
  ...
  for (i = 0; i < kernel->n_array; ++i) {
    ...
    if (local->n_pe_group == 0) continue;
 ...
 *
 * Mark all outer band nodes as atomic to ensure each kernel is only scheduled
 * once. If the domain elements that reach "node" live in more than one space,
 ...
 * systolic array.
 * [PE optimization]
 * PE optimization is applied next including: array parititioning, latency
 * hiding, and SIMD vectorization. For array partitioning, the mark "array" is
 * added between the tile and point loops. All the loops below the "array" mark
 * will be mapped to FPGA device at once. For latency hiding, SIMD
 * vectorization, all the generated loops will be marked "latency" and "SIMD".
 ...
 * "local" mark. If present, the mapping to local memory is computed at this
 * point. The "local" mark will be removed at the end of this function.
 *
 ...
 * all group tilings.
 *
 * We save a copy of the schedule that may influence the mappings to shared or
 * private memory in kernel->copy_schedule.
 *
...
static __isl_give isl_schedule_node *compute_and_comm_optimize(
  ...
  space_time_mode_json =
      cJSON_GetObjectItemCaseSensitive(space_time_json, "mode");
  ...
  if (!strcmp(space_time_mode, "auto")) {
     ...
     */
    isl_union_map *sizes =
        extract_sizes_from_str(gen->ctx, gen->options->autosa->sa_sizes);
  ...
  array_part_json =
      cJSON_GetObjectItemCaseSensitive(gen->tuning_config, "array_part");
  array_part_en_json =
      cJSON_GetObjectItemCaseSensitive(array_part_json, "enable");
  array_part_mode_json =
      cJSON_GetObjectItemCaseSensitive(array_part_json, "mode");

  array_part_L2_json =
      cJSON_GetObjectItemCaseSensitive(gen->tuning_config, "array_part_L2");
  array_part_L2_en_json =
      cJSON_GetObjectItemCaseSensitive(array_part_L2_json, "enable");
  array_part_L2_mode_json =
      cJSON_GetObjectItemCaseSensitive(array_part_L2_json, "mode");

  latency_json =
      cJSON_GetObjectItemCaseSensitive(gen->tuning_config, "latency");
  ...
  if (!single_statement) node = group_statements(node, kernel->id);
 ...
 * the kernel schedule.
 */
static __isl_give isl_multi_aff *create_from_access(
    isl_ctx *ctx, struct autosa_array_ref_group *group, int read) {
...
static isl_stat create_kernel_vars(struct autosa_kernel *kernel) {
  ...
  if (!kernel->var) return isl_stat_error;
  kernel->n_var = n;
 ...
 * back there on output.
 */
static __isl_give isl_schedule_node *add_copies(
...
static __isl_give isl_schedule_node *sa_add_copies(
  ...
  if (create_kernel_vars(kernel) < 0) node = isl_schedule_node_free(node);

  if (!single_statement) node = isl_schedule_node_parent(node);
...
__isl_give isl_schedule *sa_map_to_device(struct autosa_gen *gen,
  ...
  /* Add copy statements for the default schedule (used for correctness
   * verification). */
...
static __isl_give isl_printer *generate(__isl_take isl_printer *p,
  ...
  if (!scop) return isl_printer_free(p);

  ctx = isl_printer_get_ctx(p);
  prog = autosa_prog_alloc(ctx, scop);
  if (!prog) return isl_printer_free(p);
  ...
  if (is_legal < 0 || !is_legal) {
    ...
    p = gen->print(p, gen->prog, gen->tree, gen->hw_modules, gen->n_hw_modules,
                   gen->hw_top_module, gen->drain_merge_funcs,
                   gen->n_drain_merge_funcs, &gen->types, gen->print_user);
 ...
 */
int generate_sa(
    isl_ctx *ctx, const char *input, FILE *out, struct ppcg_options *options,
    __isl_give isl_printer *(*print)(
        __isl_take isl_printer *p, struct autosa_prog *prog,
        __isl_keep isl_ast_node *trees, struct autosa_hw_module **modules,
        int n_module, struct autosa_hw_top_module *module,
        struct autosa_drain_merge_func **drain_merge_funcs,
        int n_drain_merge_funcs, struct autosa_types *types, void *user),
    void *user) {
  struct autosa_gen gen;
  int r;
  int i;
  ...
  for (i = 0; i < gen.types.n; ++i) free(gen.types.name[i]);
  free(gen.types.name);
