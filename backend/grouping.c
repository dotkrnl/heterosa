...
static isl_stat ppcg_grouping_compute_dep(struct ppcg_grouping *grouping) {
  ...
  if (grouping->dep) return isl_stat_ok;
  ...
  if (!grouping->dep) return isl_stat_error;
 ...
 */
static void ppcg_grouping_leaf_free(int n,
                                    struct ppcg_grouping_leaf leaves[n]) {
  int i;
...
static isl_bool covers_src_and_dst(__isl_keep isl_map *map,
  ...
  isl_set_free(set1);
  isl_set_free(set2);
  if (is_subset < 0 || !is_subset) return is_subset;
...
static isl_stat check_merge(__isl_take isl_map *map, void *user) {
  ...
  if (ok >= 0 && ok) ok = isl_map_is_bijective(map);
  if (ok >= 0 && ok) ok = matches_prefix(map, data->src, data->dst);

  isl_map_free(map);

  if (ok < 0) return isl_stat_error;
  if (!ok) return isl_stat_ok;
 ...
 */
static isl_stat merge_pair(int n, struct ppcg_grouping_leaf leaves[n],
                           int pos) {
  int i;
  ...
  leaves[pos].prefix = isl_multi_union_pw_aff_union_add(leaves[pos].prefix,
                                                        leaves[pos + 1].prefix);
  for (i = pos + 1; i + 1 < n; ++i) leaves[i] = leaves[i + 1];
...
static int merge_leaves(int n, struct ppcg_grouping_leaf leaves[n],
  ...
  for (i = n - 1; i >= 0; --i) {
    ...
    if (ok < 0 && !data.merge) return -1;
    if (!data.merge) continue;
    if (merge_pair(n, leaves, i) < 0) return -1;
    --n;
    ++i;
  }
...
static isl_id *construct_group_id(struct ppcg_grouping *grouping,
  ...
  if (empty < 0) space = isl_space_free(space);
...
static __isl_give isl_schedule_constraints *remove_group_validity(
  ...
  if (n == 0) {
    joined = isl_union_map_intersect_domain(joined, isl_union_set_copy(domain));
    joined = isl_union_map_intersect_range(joined, isl_union_set_copy(domain));
  } else {
    joined = isl_union_map_eq_at_multi_union_pw_aff(
        joined, isl_multi_union_pw_aff_copy(prefix));
  }
...
static isl_stat add_groups(struct ppcg_grouping *grouping, int n,
  ...
  for (i = 0; i < n; ++i) {
    ...
    if (n_leaf < 0) return isl_stat_error;
    if (n_leaf <= 1) continue;
    ...
    upma = group_contraction_from_prefix_and_domain(grouping, leaves[i].prefix,
                                                    leaves[i].domain);
    grouping->sc =
        remove_group_validity(grouping->sc, leaves[i].prefix, leaves[i].domain);
...
static isl_stat group_subsequence(__isl_keep isl_schedule_node *node, int first,
  ...
  if (ppcg_grouping_compute_dep(grouping) < 0) return isl_stat_error;

  leaves = extract_leaves(node, first, n);
  if (!leaves) return isl_stat_error;
...
static isl_bool detect_groups(__isl_keep isl_schedule_node *node, void *user) {
  ...
  for (i = 0; i < n; ++i) {
    ...
    if (first < 0 && type == isl_schedule_node_leaf) first = i;
  }
  if (first >= 0) {
...
static void report_grouping(__isl_keep isl_union_pw_multi_aff *contraction,
  ...
  if (!options->debug->verbose) return;
...
__isl_give isl_schedule *ppcg_compute_grouping_schedule(
    __isl_take isl_schedule_constraints *sc, __isl_keep isl_schedule *schedule,
    struct ppcg_options *options) {
