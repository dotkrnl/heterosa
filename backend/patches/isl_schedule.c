 ...
 * is still alive.
 */
__isl_give isl_schedule *isl_schedule_from_schedule_tree(
    isl_ctx *ctx, __isl_take isl_schedule_tree *tree) {
  ...
  if (!schedule) goto error;

  schedule->ref = 1;
  schedule->root = tree;
  schedule->leaf = isl_schedule_tree_leaf(ctx);

  if (!schedule->leaf) return isl_schedule_free(schedule);
  return schedule;
error:
...
__isl_give isl_schedule *isl_schedule_cow(__isl_take isl_schedule *schedule) {
  ...
  if (schedule->ref == 1) return schedule;
...
isl_bool isl_schedule_plain_is_equal(__isl_keep isl_schedule *schedule1,
  ...
  if (!schedule1 || !schedule2) return isl_bool_error;
  if (schedule1 == schedule2) return isl_bool_true;
...
isl_stat isl_schedule_foreach_schedule_node_top_down(
  ...
  if (!sched) return isl_stat_error;
...
__isl_give isl_schedule *isl_schedule_gist_domain_params(
  ...
  if (!schedule || !context) goto error;
...
__isl_give isl_schedule *isl_schedule_pair(enum isl_schedule_node_type type,
  ...
  if (!schedule1 || !schedule2) goto error;
  ...
  if (disjoint < 0) filter1 = isl_union_set_free(filter1);
  if (!disjoint)
...
__isl_give isl_printer *isl_printer_print_schedule(
  ...
  if (!schedule) return isl_printer_free(p);
...
__isl_give isl_schedule *isl_schedule_dup(__isl_keep isl_schedule *sched) {
  ...
  isl_schedule *new_sched =
      isl_schedule_from_schedule_tree(isl_schedule_get_ctx(sched), tree);
