...
__isl_give isl_schedule_band *isl_schedule_band_from_multi_union_pw_aff(
  ...
  for (int i = 0; i < band->n; ++i) band->sched_pos[i] = -1;
  /* AutoSA Extended */
  band->mupa = mupa;
...
__isl_give isl_schedule_band *isl_schedule_band_dup(
  ...
  if (band->n && !dup->coincident) return isl_schedule_band_free(dup);

  for (i = 0; i < band->n; ++i) dup->coincident[i] = band->coincident[i];
  dup->permutable = band->permutable;

  /* AutoSA Extended */
  if (band->space_time) {
    dup->space_time = isl_alloc_array(ctx, enum autosa_loop_type, band->n);
    for (i = 0; i < band->n; ++i) dup->space_time[i] = band->space_time[i];
  }
  if (band->pe_opt) {
    dup->pe_opt = isl_alloc_array(ctx, enum autosa_loop_type, band->n);
    for (i = 0; i < band->n; ++i) dup->pe_opt[i] = band->pe_opt[i];
  }
  if (band->sched_pos) {
    dup->sched_pos = isl_alloc_array(ctx, int, band->n);
    for (i = 0; i < band->n; ++i) dup->sched_pos[i] = band->sched_pos[i];
  }
  /* AutoSA Extended */

  dup->mupa = isl_multi_union_pw_aff_copy(band->mupa);
  dup->ast_build_options = isl_union_set_copy(band->ast_build_options);
  if (!dup->mupa || !dup->ast_build_options) return isl_schedule_band_free(dup);

  if (band->loop_type) {
    dup->loop_type = isl_alloc_array(ctx, enum isl_ast_loop_type, band->n);
    if (band->n && !dup->loop_type) return isl_schedule_band_free(dup);
    for (i = 0; i < band->n; ++i) dup->loop_type[i] = band->loop_type[i];
  }
  if (band->isolate_loop_type) {
    dup->isolate_loop_type =
        isl_alloc_array(ctx, enum isl_ast_loop_type, band->n);
    if (band->n && !dup->isolate_loop_type) return isl_schedule_band_free(dup);
    for (i = 0; i < band->n; ++i)
...
isl_bool isl_schedule_band_plain_is_equal(__isl_keep isl_schedule_band *band1,
  ...
  if (!band1 || !band2) return isl_bool_error;
  if (band1 == band2) return isl_bool_true;

  if (band1->n != band2->n) return isl_bool_false;
  for (i = 0; i < band1->n; ++i)
    if (band1->coincident[i] != band2->coincident[i]) return isl_bool_false;
  if (band1->permutable != band2->permutable) return isl_bool_false;

  equal = isl_multi_union_pw_aff_plain_is_equal(band1->mupa, band2->mupa);
  if (equal < 0 || !equal) return equal;

  if (!band1->loop_type != !band2->loop_type) return isl_bool_false;
  if (band1->loop_type)
    for (i = 0; i < band1->n; ++i)
      if (band1->loop_type[i] != band2->loop_type[i]) return isl_bool_false;
...
__isl_give isl_schedule_band *isl_schedule_band_set_permutable(
  ...
  if (!band) return NULL;
  if (band->permutable == permutable) return band;
  band = isl_schedule_band_cow(band);
  if (!band) return NULL;
...
__isl_give isl_schedule_band *isl_schedule_band_intersect_domain(
  ...
  band = isl_schedule_band_cow(band);
  if (!band || !domain) goto error;

  band->mupa = isl_multi_union_pw_aff_intersect_domain(band->mupa, domain);
  if (!band->mupa) return isl_schedule_band_free(band);
...
enum isl_ast_loop_type isl_schedule_band_member_get_ast_loop_type(
    __isl_keep isl_schedule_band *band, int pos) {
  if (!band) return isl_ast_loop_error;

  if (pos < 0 || pos >= band->n)
    isl_die(isl_schedule_band_get_ctx(band), isl_error_invalid,
            "invalid member position", return isl_ast_loop_error);

  if (!band->loop_type) return isl_ast_loop_default;
...
__isl_give isl_schedule_band *isl_schedule_band_member_set_ast_loop_type(
  ...
  band = isl_schedule_band_cow(band);
  if (!band) return isl_schedule_band_free(band);
  ...
  if (!band->loop_type) {
    ...
    if (band->n && !band->loop_type) return isl_schedule_band_free(band);
  }
...
enum isl_ast_loop_type isl_schedule_band_member_get_isolate_ast_loop_type(
  ...
  if (!band->isolate_loop_type) return isl_ast_loop_default;
...
static __isl_give isl_union_set *add_loop_types(
  ...
  for (i = 0; i < n; ++i) {
    ...
    if (type[i] == isl_ast_loop_default) continue;

    first = i;
    while (i + 1 < n && type[i + 1] == type[i]) ++i;
...
static isl_bool is_isolate_loop_type_option(__isl_keep isl_set *set) {
  ...
  if (!isl_set_is_wrapping(set)) return isl_bool_false;
 ...
 */
static isl_bool has_isolate_loop_type_options(
...
static enum isl_ast_loop_type extract_loop_type(
  ...
  for (type = isl_ast_loop_atomic; type <= isl_ast_loop_separate; ++type) {
    ...
    if (empty < 0) return isl_ast_loop_error;
    if (empty) continue;
...
static int extract_loop_types(__isl_keep isl_schedule_band *band,
  ...
  if (!band->loop_type) {
    ...
    if (band->n && !band->loop_type) return -1;
  }
  for (i = 0; i < band->n; ++i) {
    band->loop_type[i] = extract_loop_type(options, i, 0);
    if (band->loop_type[i] == isl_ast_loop_error) return -1;
  }
...
static int extract_isolate_loop_types(__isl_keep isl_schedule_band *band,
  ...
  if (!band->isolate_loop_type) {
    ...
    band->isolate_loop_type =
        isl_alloc_array(ctx, enum isl_ast_loop_type, band->n);
    if (band->n && !band->isolate_loop_type) return -1;
  }
  for (i = 0; i < band->n; ++i) {
    band->isolate_loop_type[i] = extract_loop_type(options, i, 1);
    if (band->isolate_loop_type[i] == isl_ast_loop_error) return -1;
  }
...
__isl_give isl_schedule_band *isl_schedule_band_set_ast_build_options(
  ...
  band = isl_schedule_band_cow(band);
  if (!band || !options) goto error;
  has_isolate = has_isolate_option(options);
  if (has_isolate < 0) goto error;
  has_loop_type = has_loop_type_options(options);
  if (has_loop_type < 0) goto error;
  has_isolate_loop_type = has_isolate_loop_type_options(options);
  if (has_isolate_loop_type < 0) goto error;

  if (!has_loop_type) {
    free(band->loop_type);
    band->loop_type = NULL;
  } else {
    if (extract_loop_types(band, options) < 0) goto error;
  ...
  if (!has_isolate_loop_type) {
    ...
    if (extract_isolate_loop_types(band, options) < 0) goto error;
...
__isl_give isl_schedule_band *isl_schedule_band_replace_ast_build_option(
  ...
  if (!band->ast_build_options) return isl_schedule_band_free(band);
...
static isl_multi_union_pw_aff *isl_multi_union_pw_aff_tile(
    __isl_take isl_multi_union_pw_aff *sched, __isl_take isl_multi_val *sizes) {
  isl_ctx *ctx;
  int i;
  isl_size n;
  isl_val *v;
  ...
  if (n < 0) sched = isl_multi_union_pw_aff_free(sched);
  for (i = 0; i < n; ++i) {
    ...
    if (scale) upa = isl_union_pw_aff_scale_val(upa, isl_val_copy(v));
    isl_val_free(v);
...
__isl_give isl_schedule_band *isl_schedule_band_drop(
  ...
  if (band->pe_opt)
    for (i = pos + n; i < band->n; ++i) band->pe_opt[i - n] = band->pe_opt[i];
  if (band->sched_pos)
    for (i = pos + n; i < band->n; ++i)
...
enum autosa_loop_type isl_schedule_band_member_get_space_time(
    __isl_keep isl_schedule_band *band, int pos) {
  if (!band) return autosa_loop_error;

  if (pos < 0 || pos >= band->n)
    isl_die(isl_schedule_band_get_ctx(band), isl_error_invalid,
            "invalid member position", return autosa_loop_error);

  if (!band->space_time) return autosa_loop_error;
...
enum autosa_loop_type isl_schedule_band_member_get_pe_opt(
  ...
  if (!band->pe_opt) return autosa_loop_error;
 ...
 */
int isl_schedule_band_member_get_sched_pos(__isl_keep isl_schedule_band *band,
                                           int pos) {
  if (!band) return -1;
...
__isl_give isl_schedule_band *isl_schedule_band_member_set_sched_pos(
  ...
  if (!band->sched_pos)
    band->sched_pos =
        isl_calloc_array(isl_schedule_band_get_ctx(band), int, band->n);
